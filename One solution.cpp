...
bool solve_searching(int col);                        //Функция для поиска решения
...
void mark_under_attack(int row, int col, int val) {
    int i, j; //Инициализируем заранее что бы не инициализировать затем в 6 циклах
    ...
    // Помечаем диагонали
    // Идем от нашего ферзя влево вниз
    for (i = row, j = col; i >= 0 && j >= 0; --i, --j) {
        if (board[i][j] == 0) {
            board[i][j] = val;
        }
    }
    // Идем от нашего ферзя вправо вверх
...
bool solve_searching(int col) {
    ...
    bool found_solution = false;            // Переменная для отслеживания нахождения решения
    for (int i = 0; i < N; ++i) {
        if (check_safety(i, col)) {
            board[i][col] = 1;              //Ставим ферзя
            print_board();                  //Выводим текущую расстановку на доске
            mark_under_attack(i, col, -1);  //Помечаем клетки находящиеся под ударом
            found_solution = solve_searching(col + 1) || found_solution;
            board[i][col] = 0;
            mark_under_attack(i, col, 0);   // Сбрасываем пометку клеток
            if (found_solution) {           // Если нашли решение, завершаем цикл
                break;
            }
        }
    }
